...
| **LLM05** | Improper Output Handling | GOOD | 7/10 | Telegram rendering limits markdown image exfiltration. `streaming: { mode: "off" }` prevents duplicate message confusion. Gap: Control UI (if accessed) would be more vulnerable to markdown injection. |
