...
python3 - "$TASKS_DIR" "$PROMPT" "$PROJECT" "$PRIORITY" "$MODE" "$SESSION_ID" \
...
context_file = sys.argv[9] or None
context_content = sys.argv[10] or None
...
print(f"File: {path}")
print(f"Priority: {priority} | Mode: {mode} | Timeout: {timeout_minutes}m")
if project:
